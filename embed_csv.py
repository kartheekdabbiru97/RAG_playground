#!/usr/bin/env python3
"""
CSV Embedding and FAISS Indexing Script

This script loads a CSV file, computes sentence embeddings for each row using OpenAI API,
and saves the index in FAISS format for efficient similarity search.
"""
...
class CSVEmbedder:
    """Class to handle CSV loading, embedding generation, and FAISS indexing."""
...
def main():
    """Main function to run the CSV embedding pipeline."""
    import argparse
    
    parser = argparse.ArgumentParser(description="Load CSV, compute embeddings, and save FAISS index")
    ...
    try:
        # Initialize embedder
        embedder = CSVEmbedder(model=args.model)
